...
int run_tracker(Mat_img img, tracker_handle *tracker_data, bool detect_flag) {
  ...
  if (detect_flag)
	objects_detect_update(trackers, tracker_data->new_objs, &tracker_data->ids, img, tracker_data->tconfig);
  else
	objects_track_update(trackers, img, tracker_data->tconfig);

  out_object_tracker_info(trackers, tracker_data->tconfig, &tracker_data->trk_objs);
