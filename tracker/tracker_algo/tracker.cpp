...
void
est_histogram_rgb (Mat_img frame, Rectf roi, float *_hist, int size)
{
  int i, j;
  ...
  if (roi.x < 0)
    roi.x = 0;
  if (roi.x + roi.width >= frame.img_width)
    roi.width = frame.img_width - roi.x - 1;

  if (roi.y < 0)
    roi.y = 0;
  if (roi.y + roi.height >= frame.img_height)
    roi.height = frame.img_height - roi.y - 1;

  inv_roi_size = 1.0 / (roi.width * roi.height);

  img_ptr = frame.data[0] + ((int) (frame.width * roi.y + roi.x));
  diff_width = frame.width - roi.width;

  if ((int) roi.x % 2)
    img_ptr_uv =
        frame.data[1] + ((int) (frame.width * (((int) roi.y) / 2) + roi.x));
  else
    img_ptr_uv =
        frame.data[1] + ((int) (frame.width * (((int) roi.y) / 2) + roi.x - 1));
...
void
est_histogram_hsv (Mat_img frame, Rectf roi, float *_hist, int size)
{
  int i, j;
  unsigned char *img_ptr, *img_ptr_uv;
  int y, u = 0, v = 0;
...
float
hist_dist_correlation (float *hist1, float *hist2, int size)
{
...
float
region_overlap (Rectf b1, Rectf b2)
{
  float iou = 0;
...
float
percentage_occlusion (Rectf b1, Rectf b2)
{
  float iou = 0;
  float b1x2 = b1.x + b1.width;
  float b1y2 = b1.y + b1.height;
...
void
estimate_bbox_info (MAE_info& maInfo, Rectf prev_bbox, short duration,
    Rectf * bbox_new)
{
  bbox_info bbox_avg;
  int cnt, st_pt, ed_pt;
  ...
  if (cnt) {
    ...
    if ((fabs (x_dir) / prev_bbox.width > 0.1)
        || (fabs (y_dir) / prev_bbox.height > 0.1)) {
      ...
      if (bbox_avg.cx < 0)
        bbox_new->x =
            (cx + (bbox_avg.cx * duration) - (fabs (bbox_avg.d2cx) * duration));
      else
        bbox_new->x =
            (cx + (bbox_avg.cx * duration) + (fabs (bbox_avg.d2cx) * duration));
      ...
      if (bbox_avg.cy < 0)
        bbox_new->y =
            (cy + (bbox_avg.cy * duration) - (fabs (bbox_avg.d2cy) * duration));
      else
        bbox_new->y =
            (cy + (bbox_avg.cy * duration) + (fabs (bbox_avg.d2cy) * duration));
    } else {
...
void
ema_update_bbox (Rectf prev_bbox, Rectf * cur_bbox)
{
  cur_bbox->x = prev_bbox.x * (1.0 - LRN_POS) + LRN_POS * cur_bbox->x;
  cur_bbox->y = prev_bbox.y * (1.0 - LRN_POS) + LRN_POS * cur_bbox->y;
  cur_bbox->width =
      prev_bbox.width * (1.0 - LRN_SCALE) + LRN_SCALE * cur_bbox->width;
  cur_bbox->height =
      prev_bbox.height * (1.0 - LRN_SCALE) + LRN_SCALE * cur_bbox->height;
}

void
update_movingAverageInfo (MAE_info * maInfo, Rectf prev_bbox, Rectf cur_bbox)
{
  ...
  if (maInfo->complete) {
    ...
    maInfo->bbox_data[idx].dcx =
        maInfo->bbox_data[prev_idx].dcx * (1.0 - LRN_DIFF_POS) +
        LRN_DIFF_POS * cur_val;
    maInfo->bbox_sum.dcx += maInfo->bbox_data[idx].dcx;
    cur_val = cur_cy - prev_cy;
    maInfo->bbox_sum.dcy -= maInfo->bbox_data[idx].dcy;
    maInfo->bbox_data[idx].dcy =
        maInfo->bbox_data[prev_idx].dcy * (1.0 - LRN_DIFF_POS) +
        LRN_DIFF_POS * cur_val;
    maInfo->bbox_sum.dcy += maInfo->bbox_data[idx].dcy;
    cur_val = cur_bbox.width - prev_bbox.width;
    maInfo->bbox_sum.dw -= maInfo->bbox_data[idx].dw;
    maInfo->bbox_data[idx].dw =
        maInfo->bbox_data[prev_idx].dw * (1.0 - LRN_DIFF_SCALE) +
        LRN_DIFF_SCALE * cur_val;
    maInfo->bbox_sum.dw += maInfo->bbox_data[idx].dw;
    cur_val = cur_bbox.height - prev_bbox.height;
    maInfo->bbox_sum.dh -= maInfo->bbox_data[idx].dh;
    maInfo->bbox_data[idx].dh =
        maInfo->bbox_data[prev_idx].dh * (1.0 - LRN_DIFF_SCALE) +
        LRN_DIFF_SCALE * cur_val;
    maInfo->bbox_sum.dh += maInfo->bbox_data[idx].dh;

    cur_val = maInfo->bbox_data[idx].dcx - maInfo->bbox_data[prev_idx].dcx;
    maInfo->bbox_sum.d2cx -= maInfo->bbox_data[idx].d2cx;
    maInfo->bbox_data[idx].d2cx =
        maInfo->bbox_data[prev_idx].d2cx * (1.0 - LRN_DIFF_POS) +
        LRN_DIFF_POS * cur_val;
    maInfo->bbox_sum.d2cx += maInfo->bbox_data[idx].d2cx;
    cur_val = maInfo->bbox_data[idx].dcy - maInfo->bbox_data[prev_idx].dcy;
    maInfo->bbox_sum.d2cy -= maInfo->bbox_data[idx].d2cy;
    maInfo->bbox_data[idx].d2cy =
        maInfo->bbox_data[prev_idx].d2cy * (1.0 - LRN_DIFF_POS) +
        LRN_DIFF_POS * cur_val;
    maInfo->bbox_sum.d2cy += maInfo->bbox_data[idx].d2cy;
    cur_val = maInfo->bbox_data[idx].dw - maInfo->bbox_data[prev_idx].dw;
    maInfo->bbox_sum.d2w -= maInfo->bbox_data[idx].d2w;
    maInfo->bbox_data[idx].d2w =
        maInfo->bbox_data[prev_idx].d2w * (1.0 - LRN_DIFF_SCALE) +
        LRN_DIFF_SCALE * cur_val;
    maInfo->bbox_sum.d2w += maInfo->bbox_data[idx].d2w;
    cur_val = maInfo->bbox_data[idx].dh - maInfo->bbox_data[prev_idx].dh;
    maInfo->bbox_sum.d2h -= maInfo->bbox_data[idx].d2h;
    maInfo->bbox_data[idx].d2h =
        maInfo->bbox_data[prev_idx].d2h * (1.0 - LRN_DIFF_SCALE) +
        LRN_DIFF_SCALE * cur_val;
    maInfo->bbox_sum.d2h += maInfo->bbox_data[idx].d2h;
    cur_val =
        sqrt (maInfo->bbox_data[idx].dcx * maInfo->bbox_data[idx].dcx +
        maInfo->bbox_data[idx].dcy * maInfo->bbox_data[idx].dcy);
    maInfo->bbox_data[idx].vel =
        maInfo->bbox_data[prev_idx].vel * (1.0 - LRN_VEL) + LRN_VEL * cur_val;
  } else if (idx) {
    prev_idx = idx - 1;
...
float
match_distance (MAE_info& mae_data, Rectf prev_bbox, float *prev_hist,
    Rectf cur_bbox, float *cur_hist, int duration, track_config tconfig)
{
  ...
  float chg_in_width, chg_in_height;

  if (cur_bbox.width > est_bbox.width)
    chg_in_width = (cur_bbox.width - est_bbox.width) / cur_bbox.width;
  else
    chg_in_width = (est_bbox.width - cur_bbox.width) / est_bbox.width;

  if (cur_bbox.height > est_bbox.height)
    chg_in_height = (cur_bbox.height - est_bbox.height) / cur_bbox.height;
  else
    chg_in_height = (est_bbox.height - cur_bbox.height) / est_bbox.height;

  if (tconfig.tracker_type != ALGO_IOU || tconfig.iou_use_color) {
    if (corr < tconfig.dist_correlation_threshold ||
        iou <= tconfig.dist_overlap_threshold ||
        chg_in_width > tconfig.dist_scale_change_threshold ||
        chg_in_height > tconfig.dist_scale_change_threshold)
      return dist;
    dist = (1.0 - corr) * tconfig.dist_correlation_weight +
        (1.0 - iou) * tconfig.dist_overlap_weight +
        chg_in_width * tconfig.dist_scale_change_weight +
        chg_in_height * tconfig.dist_scale_change_weight;
  } else {
...
float
match_distance_long (MAE_info& mae_data, Rectf prev_bbox, float *prev_hist,
    float *prev_hist_st, Rectf cur_bbox, float *cur_hist, int duration,
    track_config tconfig)
{
  ...
  if (tconfig.tracker_type != ALGO_IOU || tconfig.iou_use_color) {
    ...
    if ((corr < tconfig.dist_correlation_threshold) && (mae_data.idx >= 3
            || mae_data.complete)) {
  ...
  if (iou != 0) {
    if (cur_bbox.width > prev_bbox.width)
      chg_in_width = (cur_bbox.width - prev_bbox.width) / cur_bbox.width;
    else
      chg_in_width = (prev_bbox.width - cur_bbox.width) / prev_bbox.width;

    if (cur_bbox.height > prev_bbox.height)
      chg_in_height = (cur_bbox.height - prev_bbox.height) / cur_bbox.height;
    else
      chg_in_height = (prev_bbox.height - cur_bbox.height) / prev_bbox.height;
  } else {
    estimate_bbox_info (mae_data, prev_bbox, duration, &est_bbox);
...
int
find_empty_tracker (vvas_tracker ** tracker)
{
  int idx = -1;
...
void
track_by_detection (vvas_tracker ** tracker, objs_data& new_objs, int *ids,
    Mat_img img, track_config tconfig)
{
  int i, j, k, l, min_idx;
  float iou;
  ...
  float dist, min_dist;
  float obj_hist[MAX_OBJ_TRACK][CORR_HIST_SIZE] = { 0 };
  Rectf bbox1 = { 0 }, bbox2 = { 0 };
      ...
      if (!found_det[k]) {
        ...
        for (j = 0; j < MAX_OBJ_TRACK; j++) {
          if (!found_trk[j] && tracker[j]->status >= 0) {
            ...
            dist = match_distance (tracker[j]->movingAvg_info,
                tracker[j]->prev_pos, tracker[j]->hist_map,
                bbox1, obj_hist[k], 1, tconfig);
      ...
      if (!found_det[k]) {
        if (assignment[i] >= 0) {
          if (dist_match[assignment[i] * det_num + i] != 1000.0) {
            ...
            if (tracker[trk_id]->cnt_confidence >= tconfig.num_frames_confidence
                && tracker[trk_id]->status == 0) {
              //Compare with all inactive trackers and find best match
              min_dist = 1000.0;
              min_idx = MAX_OBJ_TRACK;
              for (j = 0; j < MAX_OBJ_TRACK; j++) {
                if (!found_trk[j] && tracker[j]->status == -1 && trk_id != j) {
                  dist = match_distance_long (tracker[j]->movingAvg_info,
                      tracker[j]->prev_pos, tracker[j]->hist_map,
                      tracker[j]->hist_map_st, tracker[trk_id]->obj_rect,
                      tracker[trk_id]->hist_map,
                      tracker[j]->cnt_inactive, tconfig);
                  if (dist < min_dist) {
                    min_dist = dist;
                    min_idx = j;
                  }
              ...
              //No matching inactive tracker found assign new id
              if (min_idx == MAX_OBJ_TRACK) {
  ...
  for (j = 0; j < MAX_OBJ_TRACK; j++) {
    if (!found_trk[j]) {
      if (tracker[j]->status == -1) {
        ...
        if (tracker[j]->cnt_inactive >=
            (tconfig.num_inactive_frames / UPDATE_INTRVL))
          tracker[j]->status = -2;
...
void
vvas_tracker::init_tracker (track_config tconfig, Rectf bbox, Mat_img img,
    float *hist_new)
{
...
void
vvas_tracker::detect_update (Rectf bbox, Mat_img img, float *hist_new)
{
...
void
vvas_tracker::track_update (Mat_img img, track_config tconfig)
{
  float conf;
  Rectf bbox;
...
int
init_tracker (tracker_handle * tracker_data)
{
  ...
  for (int i = 0; i < MAX_OBJ_TRACK; i++) {
    trackers[i] =
        (vvas_tracker *) malloc (sizeof (vvas_tracker) * MAX_OBJ_TRACK);
    trackers[i]->status = -3;
    if (tracker_data->tconfig.tracker_type == ALGO_IOU)
      trackers[i]->tracker_type = 0;
    else if (tracker_data->tconfig.tracker_type == ALGO_MOSSE)
      trackers[i]->tracker_type = 1;
    else if (tracker_data->tconfig.tracker_type == ALGO_KCF)
...
int
deinit_tracker (tracker_handle * tracker_data)
{
  vvas_tracker **trackers;
...
void
out_object_tracker_info (vvas_tracker ** tracker, track_config tconfig,
    objs_data * trk_objs)
{
  int obj_cnt = 0;
  for (int i = 0; i < MAX_OBJ_TRACK; i++) {
...
int
objects_track_update (vvas_tracker ** tracker, Mat_img img,
    track_config tconfig)
{
  for (int i = 0; i < MAX_OBJ_TRACK; i++) {
    if (tracker[i]->status >= 0)
...
int
objects_detect_update (vvas_tracker ** tracker, objs_data& new_objs, int *ids,
    Mat_img img, track_config tconfig)
{
...
int
run_tracker (Mat_img img, tracker_handle * tracker_data, bool detect_flag)
{
  vvas_tracker **trackers;
  trackers = (vvas_tracker **) tracker_data->tracker_info;

  if (detect_flag)
    objects_detect_update (trackers, tracker_data->new_objs, &tracker_data->ids,
        img, tracker_data->tconfig);
  else
    objects_track_update (trackers, img, tracker_data->tconfig);

  out_object_tracker_info (trackers, tracker_data->tconfig,
      &tracker_data->trk_objs);
