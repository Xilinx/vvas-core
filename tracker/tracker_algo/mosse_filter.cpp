...
void
mosse_filter::est_gaussian_wts (int rows, int cols, fMat * dst)
{
  float *fptr;
  int j_pos;
  dst->width = cols;
  ...
  float sigma =
      std::sqrt ((float) cols * rows) / padding_scale * MOSSE_GUS_SIGMA;
...
void
mosse_filter::hanning_weights (int rows, int cols, fMat * hann_mat)
{
...
resize (Mat_img * src, fMat * dst)
{
  ...
  for (int y = 0; y < dst->height; y++) {
    ...
    for (int x = 0; x < dst->width; x++) {
      ...
      target[y * dst->width + x] =
          ((source[offset] * (FACTOR - fracx) * (FACTOR - fracy) +
              source[offset + 1] * fracx * (FACTOR - fracy) + source[offset +
                  src->width] * (FACTOR - fracx) * fracy + source[offset +
                  src->width + 1] * fracx * fracy +
...
void
mosse_filter::extract_object (Mat_img frame, bool first, float scale,
    fMat * obj)
{
  Rect new_roi;
  ...
  if (z.width != obj_size.width || z.height != obj_size.height) {
    ...
    for (int i = 0; i < obj->width * obj->height; i++)
      obj->data[i] = (float) z.data[0][i];
  }
}

void
mosse_filter::filter_response ()
{
...
void
mosse_filter::filter_response_update ()
{
#ifdef XLNX_EMBEDDED_PLATFORM
  arm_cmplx_mult_cmplx_flt_resp_upate_f32 (in_F, gau_wt, &A, &B, &filter_H,
      lrn_rate);
#endif
...
float
mosse_filter::sub_pix_estimation (float pre, float cur, float nxt)
{
...
void
mosse_filter::estimate_fourier (fMat src, fMat dst, bool fft_type)
{
#ifdef XLNX_EMBEDDED_PLATFORM
...
void
mosse_filter::apply_Hanning_window (fMat hann_win, fMat * inout)
{
#ifdef XLNX_EMBEDDED_PLATFORM
...
void
mosse_filter::init_cf (track_config tconfig, Rectf bbox_new, Mat_img frame)
{
  int size;
  int roi_size;
  ...
  rsz_z.data = (float *) (z.data[0] + roi_size);
...
void
mosse_filter::detect_update (Rectf bbox_new, Mat_img image)
{
  bbox = bbox_new;

  extract_object (image, 1, 1.0, &rsz_z);

  if (prev_obj_size.width != obj_size.width
      || prev_obj_size.height != obj_size.height) {
    hanning_weights (obj_size.height, obj_size.width, &hann_wts);
...
Point2f
mosse_filter::locate_new_position (float &peak_value)
{
...
// Update position based on the new frame
float
mosse_filter::update_position (Mat_img image, Rectf * bbox_out,
    float conf_score)
{
  if (bbox.x < 0) {
