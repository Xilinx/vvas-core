...
vvas_yolov3::vvas_yolov3 (void * handle, const std::string & model_name,
...
int
vvas_yolov3::run (void *handle, std::vector < cv::Mat > &images,
        ...
        if (kpriv->filter_labels.size ()) {
          ...
          for (unsigned int n = 0; n < kpriv->filter_labels.size (); n++) {
            ...
            if (!strncmp (current_label, filter_label, strlen (filter_label))) {
              LOG_MESSAGE (LOG_LEVEL_DEBUG, kpriv->log_level,
                  "current label %s is in filter_label list", current_label);
              found_label = true;
            }
          }
...
int
vvas_yolov3::requiredwidth (void)
{
...
int
vvas_yolov3::requiredheight (void)
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
...
int
vvas_yolov3::supportedbatchsz (void)
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
...
int
vvas_yolov3::close (void)
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
  return true;
}

vvas_yolov3::~vvas_yolov3 ()
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
}
