...
vvas_yolov2::vvas_yolov2 (void * handle, const std::string & model_name,
...
bool
compare_by_area (const vitis::ai::YOLOv2Result::BoundingBox & box1,
...
int
vvas_yolov2::run (void * handle, std::vector < cv::Mat > &images,
    VvasInferPrediction ** predictions)
{
  ...
  for (auto i = 0u; i < results.size (); i++) {
    ...
    for (auto & box:results[i].bboxes) {
        ...
        VvasInferPrediction *predict = NULL;
...
int
vvas_yolov2::requiredwidth (void)
{
...
int
vvas_yolov2::requiredheight (void)
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
...
int
vvas_yolov2::supportedbatchsz (void)
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
...
int
vvas_yolov2::close (void)
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
  return true;
}

vvas_yolov2::~vvas_yolov2 ()
{
  LOG_MESSAGE (LOG_LEVEL_DEBUG, log_level, "enter");
}
