 ...
 *         of a tree node.
 */
static bool vvas_metaaffixer_mapdata_remove_foreach (void * key, void * value, 
                                                      void * user_data)
 {
 ...
 */
static void* vvas_metaaffixer_node_scale (const void  *node, void *data)
{
 ...
 */
 static void  vvas_metaaffixer_get_inferframe_pts (VvasMetaAffixerInfo *pHandle,
                                                  VvasVideoInfo *vinfo,                                         
                                                  VvasMetadata *metadata) 
 {
    if((NULL == pHandle) ||
    ...
    while(vvas_hash_table_iter_next(&iter, (void **)&near_pts, (void **)&frame)) {
       ...
       if((infer_meta_spts >= inframe_spts) &&
          (infer_meta_spts < inframe_epts) &&
          (0 != infer_meta_dur)) {
            /* overlap present calculate overlap w.r.t to Infer meta frame */
            ovl_per = (uint32_t)round(((float)(inframe_epts - infer_meta_spts)/ infer_meta_dur) * 100);
            LOG_D("Infer<Input: frame ovlerlaps %d  Nearest PTS:%ld to inframepts:%ld ", ovl_per, *near_pts, inframe_spts);
            
         }
         /* if infer frame  is behind of inframe */
         else if ((inframe_spts >= infer_meta_spts) &&
                 (inframe_spts < infer_meta_epts) &&
                 (0 != infer_meta_dur)) {
            /* overlap present calculate overlap w.r.t to Infer meta frame */
            ovl_per =  (uint32_t)round(((float)(infer_meta_epts - inframe_spts)/ infer_meta_dur) * 100);
            LOG_D("Infer>Input: frame ovlerlaps %d  Nearest PTS:%ld to inframepts:%ld ", ovl_per, *near_pts, inframe_spts);
         }
         ...
         if(ovl_per > overlap_percent) {
           ...
           LOG_I("frame ovlerlaps %d  Nearest PTS:%ld to inframepts:%ld ", ovl_per, *near_pts, inframe_spts);
        }
 ...
 */
static void vvas_metaaffixer_compute_scale_factor(VvasInferScaleFactor *scl_factor)
 {
     if(NULL != scl_factor) {       
 ...
 */
static bool vvas_metaaffixer_print(const VvasTreeNode  *node, void *data)
{
   VvasInferPrediction *p = (VvasInferPrediction *) node->data;
   VvasMetaAffixerInfo *pHandle = (VvasMetaAffixerInfo *) data; 
   LOG_I("x=%d,y=%d, w=%d, h=%d",  p->bbox.x,    p->bbox.y,    p->bbox.width,   p->bbox.height);
 ...
 *
 */
static bool vvas_metaaffixer_node_assign(const VvasTreeNode  *node, void *data)
{
  VvasInferPrediction *dmeta = NULL;
 ...
 *  @brief This function returns scaled metadata cored on input frame info
 */
static VvasInferPrediction* vvas_metaaffixer_get_scaled_meta(VvasInferPrediction *smeta,
                                                           VvasInferScaleFactor *scl_factor,
                                                           VvasMetaAffixerInfo *pHandle)
 {
   VvasInferPrediction *dmeta = NULL;
    ...
    if((NULL != smeta) && 
       ...
       (NULL != pHandle)) {
        ...
        VvasTreeNode *node = vvas_treenode_copy_deep(smeta->node, vvas_metaaffixer_node_scale, scl_factor);
 ...
 */
static void vvas_metaaffixer_remove_infer_meta(VvasMetaAffixerInfo *pHandle)
 {
    uint32_t *pts = NULL; 
   ...
   while((vvas_hash_table_iter_next(&iter, (void **)&pts, (void **)&vmeta) &&
    ...
    if(NULL != key ) {
      ...
      VvasMetaAffixerMapData *mp = (VvasMetaAffixerMapData *) 
                          vvas_hash_table_lookup(pHandle->map, *key);
 ...
 */
                                          ...
                                          VvasLogLevel loglevel)
 {
    ...
    if((0 == inferframe_dur) ||
       (0xFFFFFFFFFFFFFFFF == inferframe_dur)||
       (0 == infer_queue_size)) {
      LOG_E("Invalid input param received");
      return NULL;      
    }
 ...
 void vvas_metaaffixer_destroy(VvasMetaAffixer* handle)
 {
    ...
    if(NULL != pHandle) {
     ...
     vvas_hash_table_foreach_remove(pHandle->map, 
                                    vvas_metaaffixer_mapdata_remove_foreach,
                                    NULL);
 ...
 *  @param [in] sync_infer_lastpts - if TRUE then last received infer meta data  
  ...
  *respcode = VVAS_METAAFFIXER_PASS;
  ...
  if(sync_infer_lastpts) {
   ...
   if(NULL != pHandle->near_pts) {
     ...
     *ScaledMetaData = vvas_metaaffixer_get_scaled_meta(mp->meta, &scl_factor, pHandle);  
